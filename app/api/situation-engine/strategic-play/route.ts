...
RED TEAM (ΚΟΡΥΦΑΙΑΣ ΣΗΜΑΣΙΑΣ — ΟΧΙ GENERIC):
Σκέψου σαν ο ΠΙΟ ΕΠΙΚΙΝΔΥΝΟΣ αντίπαλος σύμβουλος. Δώσε 3 ΣΥΓΚΕΚΡΙΜΕΝΕΣ επιθέσεις που θα δεχτεί το ${party_name} για ΑΥΤΟ ΑΚΡΙΒΩΣ το γεγονός «${activeTitle}». Κάθε επίθεση ΠΡΕΠΕΙ:
- Να έχει ΣΥΓΚΕΚΡΙΜΕΝΟ επιτιθέμενο (πραγματικός αντίπαλος: π.χ. Νέα Δημοκρατία/κυβέρνηση, ΠΑΣΟΚ, ΚΚΕ, ΜΜΕ/δημοσιογράφος, ή εσωτερική κριτική) — ΟΧΙ αόριστο «οι αντίπαλοι».
- Να περιέχει την ΠΡΑΓΜΑΤΙΚΗ ατάκα/επιχείρημα που θα ειπωθεί, αναφερόμενη ΡΗΤΑ στο γεγονός «${activeTitle}» — ΟΧΙ γενικότητες που ταιριάζουν σε οποιοδήποτε θέμα.
- Να έχει ΕΤΟΙΜΗ, αιχμηρή απάντηση/αντεπίθεση στη φωνή του ${party_name} (μία πρόταση, χρησιμοποιήσιμη αμέσως).
- Να έχει επίπεδο ρίσκου: "high" | "medium" | "low".
ΑΠΑΓΟΡΕΥΟΝΤΑΙ φράσεις όπως «θα μας κατηγορήσουν ότι εκμεταλλευόμαστε το θέμα» χωρίς συγκεκριμένο επιτιθέμενο και συγκεκριμένη ατάκα. Κάθε attack πρέπει να μπορεί να ειπωθεί ΑΥΤΟΛΕΞΕΙ από τον αντίπαλο. ΚΑΘΕ attack ΚΑΙ response: ΜΙΑ σύντομη, αιχμηρή/έξυπνη πρόταση (<20 λέξεις) — ΟΧΙ παράγραφοι.
...
ΕΠΕΣΤΡΕΨΕ ΜΟΝΟ έγκυρο JSON με ΑΚΡΙΒΩΣ αυτή τη μορφή (χωρίς code fences, χωρίς σχόλια):
{
  ...
  "red_team": [
    { "attacker": "ποιος επιτίθεται (συγκεκριμένος)", "attack": "η επίθεση σε ΜΙΑ σύντομη αιχμηρή πρόταση (<20 λέξεις), αυτολεξεί", "response": "η απάντηση σε ΜΙΑ σύντομη έξυπνη πρόταση (<20 λέξεις)", "risk_level": "high|medium|low" },
    { "attacker": "...", "attack": "...", "response": "...", "risk_level": "medium" },
    { "attacker": "...", "attack": "...", "response": "...", "risk_level": "low" }
  ],
  "options": {
...
Η A είναι η προτεινόμενη (υψηλότερο success), η C προς αποφυγή (χαμηλότερο success).`;
    ...
    // RED TEAM — επεξεργασία με εγγύηση (ΠΟΤΕ κενό).
    const riskLevel = (v: unknown): "high" | "medium" | "low" => {
      const t = String(v || "").toLowerCase();
      if (t.includes("high") || t.includes("υψη")) return "high";
      if (t.includes("low") || t.includes("χαμη")) return "low";
      return "medium";
    };
    const rawRedTeam = Array.isArray(parsed?.red_team) ? parsed.red_team : [];
    let redTeam = rawRedTeam
      .map((rt: any) => ({
        attacker: cleanStr(rt?.attacker, ""),
        attack: cleanStr(rt?.attack, ""),
        response: cleanStr(rt?.response, ""),
        risk_level: riskLevel(rt?.risk_level),
      }))
      .filter((rt: any) => rt.attacker && rt.attack)
      .slice(0, 4);

    // ΕΓΓΥΗΣΗ: αν το AI δεν έδωσε red team, βάλε συγκεκριμένο fallback για ΑΥΤΟ το γεγονός.
    if (redTeam.length === 0) {
      redTeam = [
        {
          attacker: "Κυβέρνηση / Νέα Δημοκρατία",
          attack: `«Το ${party_name} κάνει αντιπολίτευση στο «${activeTitle}» χωρίς δική του πρόταση — μόνο κριτική.»`,
          response: "Η πρότασή μας είναι συγκεκριμένη και κατατεθειμένη· το ζήτημα είναι αν η κυβέρνηση έχει το θάρρος να την εφαρμόσει.",
          risk_level: "high" as const,
        },
        {
          attacker: "ΜΜΕ / σχολιαστές",
          attack: `«Το ${party_name} εκμεταλλεύεται το «${activeTitle}» για επικοινωνιακό όφελος.»`,
          response: "Δεν μιλάμε για εμάς· μιλάμε για τους πολίτες που πληρώνουν το κόστος — και αυτό απαιτεί απάντηση, όχι σιωπή.",
