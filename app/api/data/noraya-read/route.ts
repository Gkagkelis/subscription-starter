...
function cacheKey(topicKey: string | null, topicLabel: string): string {
  return "noraya_read_v4__" + (topicKey || norm(topicLabel));
}
...
- Ιστορική τάση: ${fmtTrend(d?.trend)}`
    : `ΔΕΝ υπάρχουν σκληρά δεδομένα έρευνας γι' αυτό το θέμα. Βασίσου σε ζωντανή αναζήτηση και στη γνώση σου για την ελληνική κοινή γνώμη.`;

  return `Είσαι κορυφαίος αναλυτής κοινής γνώμης. Η ΜΟΝΗ σου δουλειά εδώ είναι ΚΑΘΑΡΗ, ΠΟΙΟΤΙΚΗ ΕΡΜΗΝΕΙΑ: ποιον ΑΓΓΙΖΕΙ αυτό το θέμα και ΤΙ ΠΙΣΤΕΥΕΙ ο κόσμος.

ΑΥΣΤΗΡΟΤΑΤΟΣ ΚΑΝΟΝΑΣ: ΜΗΝ προτείνεις ΤΙΠΟΤΑ για το τι πρέπει να κάνει το κόμμα. ΚΑΜΙΑ στρατηγική, κίνηση, «άνοιγμα», «κίνδυνος», «επόμενη κίνηση», «κινητοποίηση». ΜΟΝΟ περιγραφή και ερμηνεία των δεδομένων και της διάθεσης του κόσμου. (Η στρατηγική γίνεται αλλού.)
...
- Ψάξε ζωντανά ΕΓΚΥΡΕΣ ελληνικές πηγές για: (α) την ΑΡΙΣΤΕΡΑ-ΔΕΞΙΑ τοποθέτηση/διάθεση πάνω στο θέμα (δημοσκόποι όπως MRB, Metron Analysis, Marc, Pulse, Kapa Research, Prorata, Alco, GPO, ή σοβαρά δημοσιεύματα)· (β) αν ΔΕΝ υπάρχουν σκληρά δεδομένα, συμπληρωματικά ΠΕΡΙΓΡΑΦΙΚΑ ευρήματα για το ποιον αφορά.
- ΑΥΣΤΗΡΟΣ ΚΑΝΟΝΑΣ ΤΙΜΙΟΤΗΤΑΣ: ΜΗΝ επινοείς κανένα νούμερο ή ισχυρισμό. Κάθε νούμερο/ισχυρισμός ΠΡΕΠΕΙ να προέρχεται από πηγή που όντως βρήκες τώρα, και να μπει στο "sources" με ΠΡΑΓΜΑΤΙΚΟ URL. Αν δεν βρεις, γράψε «δεν βρέθηκαν αξιόπιστα στοιχεία».

ΓΡΑΨΕ ΣΥΝΤΟΜΑ, ΠΥΚΝΑ, ΠΕΡΙΓΡΑΦΙΚΑ. Ελληνικά, χωρίς markdown.
ΚΡΙΣΙΜΟ: Η ΑΠΑΝΤΗΣΗ ΣΟΥ ΠΡΕΠΕΙ ΝΑ ΕΙΝΑΙ ΜΟΝΟ ΤΟ JSON - να ξεκινάει με { και να τελειώνει με }. Καμία περιγραφή της αναζήτησης. Μορφή:
{
  "headline": "Η κοινωνική εικόνα σε ΜΙΑ πρόταση: ποιον αγγίζει το θέμα, η ουσία.",
  "interpretation": "1-3 προτάσεις: τι δείχνουν τα νούμερα - ποιες ομάδες, το μοτίβο, τι αποκαλύπτει για τη διάθεση του κόσμου.",
  "groups_affected": "Ποιες ομάδες αφορά και γιατί, με τα ποσοστά. Περιγραφικά.",
  "trust_read": "Τι δείχνει η εμπιστοσύνη αυτών των ομάδων στην κυβέρνηση - περιγραφικά, ΟΧΙ ως ευκαιρία/στρατηγική.",
  "web_findings": "1-3 προτάσεις με ό,τι βρήκες ζωντανά, με αναφορά στην πηγή· ή «δεν βρέθηκαν αξιόπιστα στοιχεία».",
  "left_right": "η αριστερά-δεξιά τοποθέτηση/διάθεση στο θέμα, από πηγές, σύντομα· ή «δεν βρέθηκαν αξιόπιστα στοιχεία».",
