...
const STAGES = ["Παρακολούθηση", "Χαμηλή επίπτωση", "Θεσμική πρόκληση", "Κοινωνική πίεση", "Παρέμβαση αρχηγού", "Σύγκρουση"];
...
function riskMeta(raw: unknown): { label: string; dot: string; ring: string } {
  ...
  if (/(high|υψηλ|critical|κρίσιμ|severe)/.test(v)) return { label: "Υψηλό ρίσκο", dot: "#f87171", ring: "border-red-400/40" };
  if (/(medium|μεσα|moderate|μέτρι)/.test(v)) return { label: "Μεσαίο ρίσκο", dot: "#fbbf24", ring: "border-amber-300/30" };
  if (/(low|χαμηλ|minor)/.test(v)) return { label: "Χαμηλό ρίσκο", dot: "#34d399", ring: "border-emerald-300/25" };
...
function toCard(s: Situation): Card {
  const score = num(pick(s, "priority_score") ?? pick(s, "strategic_index_score") ?? pick(s, "event_score") ?? pick(s, "agenda_score"));
  const stage = clamp(Math.round(num(pick(s, "escalation_level"), score >= 70 ? 3 : score >= 50 ? 2 : 1)), 1, 6);
  const recommended = clamp(Math.round(num(pick(s, "escalation_recommended"), score >= 75 ? 3 : 2)), 1, 6);
...
export default function SituationsPage() {
  ...
  const filtered = useMemo(
    () => cards.filter((c) => (riskFilter === "all" || c.risk.label === "Υψηλό ρίσκο") && (!onlyEscalate || c.escalate)),
      ...
      <header className="sticky top-0 z-20 border-b border-[#101a30] bg-[#060a14]/90 backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-6 py-4">
          ...
          <nav className="flex items-center gap-1">
            {navTabs.map((tab) => {
              const active = tab.label === "Καταστάσεις";
      ...
      <main className="mx-auto max-w-[1600px] px-6 py-8">
        <section className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-zinc-50 md:text-4xl">Πού στέκεται κάθε μάχη</h1>
            <p className="mt-2 max-w-2xl text-sm text-zinc-400">Κάθε ενεργή κατάσταση στο στάδιο αντίδρασης που βρίσκεται — και η επόμενη κίνηση για {partyLabel}.</p>
          </div>
        ...
        <section className="mb-5 flex flex-wrap items-center gap-2">
          ...
          <Toggle active={riskFilter === "all"} onClick={() => setRiskFilter("all")}>Όλες</Toggle>
          <Toggle active={riskFilter === "high"} onClick={() => setRiskFilter("high")}>Μόνο υψηλό ρίσκο</Toggle>
          <Toggle active={onlyEscalate} onClick={() => setOnlyEscalate((v) => !v)}>Μόνο όσες θέλουν κλιμάκωση</Toggle>
        </section>

        {loading ? (
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
            {STAGES.map((_, i) => <div key={i} className="h-72 animate-pulse rounded-3xl border border-[#1a2640] bg-[#0c1220]" />)}
          </div>
        ) : totalActive === 0 ? (
          <div className="rounded-3xl border border-[#1a2640] bg-[#0c1220] p-8 text-center text-sm text-zinc-400">
            Δεν υπάρχουν ενεργές καταστάσεις αυτή τη στιγμή. Μόλις εντοπιστούν νέα γεγονότα, ο πίνακας γεμίζει αυτόματα.
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            {STAGES.map((stage, i) => {
              ...
              const col = columns[i];
              return (
                <div key={stage} className="flex min-h-[120px] flex-col rounded-3xl border border-[#141f38] bg-[#0a0f1c]/70 p-2.5">
                  <div className="mb-2 px-1.5">
                    <div className="flex items-center justify-between">
                      <div className="text-[11px] font-semibold text-zinc-200">{stage}</div>
                      <div className="rounded-full bg-white/[0.05] px-1.5 py-0.5 text-[10px] text-zinc-400">{col.length}</div>
                    </div>
                    <div className="mt-1.5 h-[3px] w-full overflow-hidden rounded-full bg-white/[0.04]">
                      <div className="h-full origin-left rounded-full" style={{ background: accent, animation: "barGrow .7s ease forwards", animationDelay: `${i * 60}ms` }} />
                    </div>
                  </div>

                  <div className="flex flex-1 flex-col gap-2">
                    {col.length === 0 ? (
                      <div className="flex flex-1 items-center justify-center rounded-2xl border border-dashed border-[#172339] py-6 text-[10px] text-zinc-600">—</div>
                    ) : (
                      col.map((c, idx) => (
                        <button
                          key={c.id}
                          type="button"
                          onClick={() => openTopic(c)}
                          style={{ animation: "sitIn .45s ease both", animationDelay: `${idx * 50 + i * 40}ms`, borderLeftColor: c.risk.dot }}
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="flex shrink-0 items-center gap-1 text-[10px] text-zinc-500">
                              <span className="h-1.5 w-1.5 rounded-full" style={{ background: c.risk.dot }} />
                              {c.score}
                            </span>
                          </div>
                          <div className="mt-1 line-clamp-2 text-[13px] font-medium leading-5 text-zinc-100">{c.title}</div>

                          <div className="mt-2 rounded-xl bg-white/[0.03] px-2 py-1.5">
                            <div className="text-[9px] uppercase tracking-wide text-zinc-600">Επόμενη κίνηση</div>
                            <div className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-zinc-300">{c.nextMove}</div>
                          </div>

                          {c.escalate ? (
                            <div className="mt-2 flex items-center gap-1.5 rounded-lg border border-amber-300/30 bg-amber-300/10 px-2 py-1 text-[10px] text-amber-200" style={{ animation: "sitPulse 2.2s ease-in-out infinite" }}>
                              <span>▲</span>
                          ...
                          <div className="mt-2 flex items-center justify-between text-[10px] text-zinc-600">
                            <span>{c.articles} άρθρα · {c.sources} πηγές</span>
                            <span className="text-cyan-300/0 transition group-hover:text-cyan-300/70">Άνοιγμα →</span>
                          </div>
                        </button>
                      ))
                    )}
...
function Toggle({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) {
  return (
    <button type="button" onClick={onClick} className={`rounded-full border px-3 py-1.5 text-[11px] transition ${active ? "border-cyan-300/30 bg-cyan-300/10 text-cyan-100" : "border-[#1a2640] bg-[#0c1220] text-zinc-400 hover:text-zinc-200"}`}>
      {children}
    </button>
  );
}
